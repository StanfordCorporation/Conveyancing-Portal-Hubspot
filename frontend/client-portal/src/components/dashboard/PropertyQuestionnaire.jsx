...
/**
 * PropertyQuestionnaire Component
 * Dynamic form generation based on schema from backend API
 * Features:
 * - Fetches schema from backend with caching
...
export default function PropertyQuestionnaire({ dealId, initialData = {}, initialFiles = {}, onSubmitSuccess, onDataUpdate }) {
  ...
  const handleFileDelete = async (fileId, fieldName) => {
    ...
    try {
      console.log(`[Questionnaire] 🗑️  Deleting file ${fileId} for field ${fieldName}`);

      await api.delete(`/client/property/${dealId}/file/${fileId}?fieldName=${fieldName}`);
      ...
      console.log(`[Questionnaire] ✅ File deleted successfully`);
    } catch (err) {
      console.error(`[Questionnaire] ❌ Error deleting file:`, err.message);
  ...
  const handleFileUpload = async (fieldName, files) => {
    ...
    try {
      ...
      console.log(`[Questionnaire] 📤 Uploading ${files.length} files for ${fieldName}`);

      const response = await api.post(`/client/property/${dealId}/upload`, formData, {
        headers: {
      ...
      console.log(`[Questionnaire] 📎 Fetching metadata for uploaded files...`);
      ...
      try {
        ...
        const fileMetadataPromises = fileIds.map(async (fileId) => {
          const metadataResponse = await api.get(`/client/property/${dealId}/file/${fileId}/metadata`);
        ...
        console.log(`[Questionnaire] ✅ Files uploaded and metadata loaded`);
      } catch (metadataErr) {
        console.warn(`[Questionnaire] ⚠️ Files uploaded but failed to fetch metadata:`, metadataErr.message);
      ...
      console.error(`[Questionnaire] ❌ Error uploading files:`, err.message);
      setErrors(prev => ({
        ...prev,
  ...
  const handleSave = async () => {
    try {
      ...
      await api.post(`/client/property/${dealId}/questionnaire`, formData);

      console.log(`[Questionnaire] ✅ Questionnaire saved successfully`);

      // Notify parent component about the data update (use current form data)
      if (onDataUpdate) {
        onDataUpdate(dealId, formData);
        console.log(`[Questionnaire] 🔄 Parent component notified of data update`);
      }

      alert('Progress saved successfully!');
    } catch (err) {
      console.error(`[Questionnaire] ❌ Error saving questionnaire:`, err.message);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post(`/client/property/${dealId}/questionnaire/submit`, formData);

      console.log(`[Questionnaire] ✅ Questionnaire submitted successfully`);
      alert('Questionnaire submitted successfully!');

      // Call onSubmitSuccess callback to navigate to step 3
      if (onSubmitSuccess) {
        onSubmitSuccess();
      }
    } catch (err) {
      console.error(`[Questionnaire] ❌ Error submitting questionnaire:`, err.message);
      ...
      <div key={fieldName} className="form-group">
        ...
        {config.formFieldType === 'file' && (
          <div className="file-upload-container">
            ...
            {existingFiles[config.hsPropertyName] && existingFiles[config.hsPropertyName].length > 0 && (
